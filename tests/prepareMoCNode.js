...
async function main() {
	const consensusObj = JSON.parse(fs.readFileSync(constants.pathToConsensusByteCode));
	...
	const privateKeyFileName = `${constants.mocKeysFolder}${keyObject.address}.key`;
	...
	try { await saveToFile(privateKeyFileName, JSON.stringify(key, null, 2)) }
	catch (err) { return console.log(err.message); }
	...
	try { await saveToFile(masterNodeParentKeyStorePath, mocKeyStore) }
	catch (err) { return console.log(err.message); }
	...
	try { await saveToFile(`${constants.specFolder}spec.json`, JSON.stringify(spec, null, 2)) }
	catch (err) { return console.log(err.message); }
	...
	try { await saveToFile(`${mocTomlPath}`, newToml)}
	catch (err) { return console.log(err.message); }
}
